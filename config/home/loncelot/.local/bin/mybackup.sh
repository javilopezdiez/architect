...
main() {
    ...
    #     if ! sudo pacman -S bash-completion; then
    #         echo "Failed to install bash-completion. Exiting."
    #         exit 1
    #     fi
    # fi
...
copy() {
    ...
    echo "The backup files are/are tobe in: $C"
    
    if [[ "$direction" == "backup" ]]; then
        for source_dir in "${source_dirs[@]}"; do
            for expanded_source in $source_dir; do
                if [[ "$expanded_source" == $HOME* ]]; then
                    backup_dir="${expanded_source/$HOME/$CHOME}"
                else
                    backup_dir="${C}${expanded_source}"
                fi
        ...
        for source_dir in "${source_dirs[@]}"; do
            for expanded_source in $source_dir; do
                base_source_dir="${expanded_source%/*}" 
                if [[ "$expanded_source" == $HOME* ]]; then
                    backup_dir="${base_source_dir/$HOME/$CHOME}"
                else
                    backup_dir="${C}${base_source_dir}"
                fi
                sudo mkdir -p "$(dirname "$base_source_dir")"
                if [[ -d "$backup_dir" ]]; then
                    sudo rsync -ah --info=progress2 "$backup_dir/" "$base_source_dir"
                else
                    sudo rsync -ah --info=progress2 "$backup_dir" "$base_source_dir"
                fi
            done
